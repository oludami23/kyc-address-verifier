/**
 * RAG Compliance Layer — Phase C
 *
 * Retrieves the most relevant Nigerian KYC/AML compliance chunks for a given
 * verification context, then passes them to the verdict reasoning prompt so
 * Claude can cite specific regulations rather than speaking generically about
 * "the 90-day requirement".
 *
 * Architecture:
 *   1. Knowledge base — curated compliance excerpts compiled from CBN circulars,
 *      NIMC guidelines, and the CBN AML/CFT Regulations 2022.
 *   2. Indexer — TF-IDF weighting computed at module initialisation (~1ms for
 *      this corpus size; no build step or vector database required).
 *   3. Retriever — cosine-style TF-IDF similarity returns top-K chunks for a
 *      free-text query built from the verification context.
 *
 * Production upgrade path: swap the TF-IDF scorer for dense embeddings
 * (Voyage AI or all-MiniLM-L6-v2) and a Chroma/Qdrant vector store to gain
 * semantic similarity across paraphrased regulatory text.
 */

import type { Check, DocumentType, PoAType, Decision } from "./types";
...
export interface ComplianceChunk {
  ...
  section: string;       // Section / article heading
  text: string;          // Chunk content
  keywords: string[];    // Curated high-signal terms for TF-IDF boosting
}
...
// Sourced from: CBN Guidelines on Mobile Money Services (Revised 2015),
// CBN AML/CFT Regulations 2022, NIMC Act 2007, CBN Consumer Protection
// Framework 2022, CBN Regulatory Framework for BVN Operations 2014.
// ---------------------------------------------------------------------------

const KNOWLEDGE_BASE: ComplianceChunk[] = [
...
function buildIDF(chunks: ComplianceChunk[]): Map<string, number> {
...
function scoreChunk(queryTerms: string[], chunk: ComplianceChunk): number {
  const chunkTerms = tokenise(chunk.text);
  const chunkLen = chunkTerms.length;

  // TF: term frequency in chunk (with keyword boost ×2)
  const tf = new Map<string, number>();
  for (const t of chunkTerms) tf.set(t, (tf.get(t) ?? 0) + 1);
  for (const k of chunk.keywords) tf.set(k, (tf.get(k) ?? 0) + 2); // keyword boost
...
export function buildRetrievalQuery(
  ...
  // Document types
  if (idType === "NIN") parts.push("NIN national identification number verification");
  if (idType === "DRIVERS_LICENSE") parts.push("driver licence expiry validity KYC");

  if (poaType === "UTILITY_BILL") parts.push("utility bill proof of address recency");
  if (poaType === "BANK_STATEMENT") parts.push("bank statement proof of address");

  // Check results drive the query toward relevant compliance chunks
  const checkMap = Object.fromEntries(checks.map((c) => [c.name, c.status]));

  if (checkMap["Name match"] === "FAIL") parts.push("name mismatch identity document escalate");
  if (checkMap["Name match"] === "WARN") parts.push("partial name match abbreviated Nigerian naming");
  if (checkMap["Document recency"] === "WARN" || checkMap["Document recency"] === "FAIL")
    parts.push("proof of address recency 90 days CBN requirement");
  if (checkMap["Document authenticity signals"] !== "PASS")
  ...
  if (decision === "REVIEW_REQUIRED") parts.push("review required customer due diligence");
  if (decision === "REJECTED") parts.push("rejected enhanced due diligence AML");
...
const RELEVANCE_SUMMARIES: Record<string, string> = {
  "cbn-tier-1": "KYC Tier 1 transaction and balance limits",
  "cbn-tier-2": "KYC Tier 2 identity document requirements",
  "cbn-tier-3": "KYC Tier 3 full KYC including proof-of-address requirement",
  "poa-recency": "Proof-of-address 90-day recency requirement",
  "poa-acceptable-docs": "Acceptable proof-of-address document types",
  "poa-content-requirements": "Required content in proof-of-address documents",
  "id-nin": "NIN verification standards and format",
  "id-drivers-licence": "Driver's licence validity and expiry check",
  "id-passport": "International passport acceptance criteria",
  "name-matching": "Name matching tolerances and mismatch escalation",
  "name-nigerian-patterns": "Nigerian naming conventions (titles, hyphens, single names)",
  "aml-cdd": "Standard Customer Due Diligence requirements",
  "aml-enhanced-cdd": "Enhanced Due Diligence triggers",
  "authenticity-standards": "Document authenticity red flags and escalation",
  "authenticity-digital-docs": "Digital and scanned document acceptance standards",
};

export function retrieveCompliance(query: string, topK = 4): RetrievalResult[] {
  ...
  const scored = KNOWLEDGE_BASE.map((chunk) => ({
    ...
    relevance_summary: RELEVANCE_SUMMARIES[chunk.id] ?? chunk.section,
  }));
...
// ComplianceChunk and RetrievalResult are already exported via their interface declarations above.
